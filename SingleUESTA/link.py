class Link:
    ...
    def calculateMarginalCost(self):
        """
        Calculates marginal cost d/dx(x*t(x)) for use in SO assignments.
        """
        if self.capacity <= 0:
            return self.freeFlowTime
        vcRatio = self.flow / self.capacity
        if vcRatio <= 0:
            return self.freeFlowTime
        return self.freeFlowTime * (1 + self.alpha * (1 + self.beta) * pow(vcRatio, self.beta))

    def updateMarginalCost(self):
        self.marginalCost = self.calculateMarginalCost()
