...
class Network:
    ...
    def FrankWolfeStepSize(self, targetFlows, precision = FRANK_WOLFE_STEPSIZE_PRECISION, useMarginalCosts = False):
        """
        ...
        while (high-low) > precision:
            ...
            for ij in self.link:
                ...
                if useMarginalCosts:
                    linkCost = self.link[ij].calculateMarginalCost()
                else:
                    linkCost = self.link[ij].calculateCost()
                derivative += (targetFlows[ij] - currentFlows[ij]) * linkCost
    ...
    def userEquilibriumFW(self,
        ...
        while self.iteration < maxIterations:
            ...
            stepSize = self.FrankWolfeStepSize(targetFlows, useMarginalCosts = True)
            ...
            shortestFlows = self.allOrNothing()
            SPTT = 0
            TSTT = 0
            for ij in self.link:
                SPTT += self.link[ij].cost * shortestFlows[ij]
                TSTT += self.link[ij].cost * self.link[ij].flow
            currentGap = TSTT / SPTT - 1
            objective = self.beckmannFunction()
            self.convergenceHistory.append({
                "iteration": self.iteration,
                "gap": currentGap,
                "objective": objective
            })
            print("Iteration %d: gap %f; obj fun %f" % (self.iteration, currentGap, objective))
            if currentGap < targetGap:
                break

        return objective, TSTT
        

    def systemOptimalFW(self,
                        maxIterations = 100,
                        targetGap = 1e-4):
        """
        Single-class system optimal STA solved via Frank-Wolfe with marginal costs.
        """
        initialFlows = self.allOrNothing()
        for ij in self.link:
            self.link[ij].flow = initialFlows[ij]
        ...
        while self.iteration < maxIterations:
            ...
            trueCosts = {ij: self.link[ij].cost for ij in self.link}
            marginalCosts = dict()
            for ij in self.link:
                mCost = self.link[ij].calculateMarginalCost()
                marginalCosts[ij] = mCost
                self.link[ij].cost = mCost

            targetFlows = self.allOrNothing()
            for ij in self.link:
                self.link[ij].cost = trueCosts[ij]
            ...
            # Evaluate optimality gap using marginal costs
            marginalCostsAfter = dict()
            for ij in self.link:
                marginalCostsAfter[ij] = self.link[ij].calculateMarginalCost()

            # Temporarily set link costs to marginal to find marginal shortest paths
            trueCostsEval = {ij: self.link[ij].cost for ij in self.link}
            for ij in self.link:
                self.link[ij].cost = marginalCostsAfter[ij]
            shortestFlows = self.allOrNothing()
            for ij in self.link:
                self.link[ij].cost = trueCostsEval[ij]

            SPTT = 0
            TSTT = 0
            for ij in self.link:
                SPTT += marginalCostsAfter[ij] * shortestFlows[ij]
                TSTT += marginalCostsAfter[ij] * self.link[ij].flow

            currentGap = TSTT / max(SPTT, 1e-12) - 1
            self.convergenceHistory.append({
                "iteration": self.iteration,
                "gap": currentGap,
                "objective": TSTT
            })
            print("Iteration %d: gap %f; obj fun %f" % (self.iteration, currentGap, TSTT))
    ...
    def readDemandFile(self, demandFileName):
        ...
        try:
            ...
            print("\nError reading network file %s" % demandFile)
            traceback.print_exc(file=sys.stdout)
